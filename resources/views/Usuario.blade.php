...
@section('conteudo')
    <main>
        ...
        @if($sucesso != '')
            <div style="background-color: rgba(0, 255, 0, 0.63); padding: 10px; border-radius: 5px;">
                <p style="color: white; font-weight: bold;">{{ $sucesso }}</p>
            </div>
        @endif
        <form action="/usuario" class="container-form" method="post">
            ...
            <label for="nome_user" class="rotulo">
                Nome de usuário
                <input type="text" class="input-dado" name="nome_user" id="nome_user"
                       value="{{ session()->get('usuario')->nome; }}">
            </label>

            <label for="oab_user" class="rotulo">
                OAB
                <input type="text" class="input-dado" name="oab_user" id="oab_user"
                       value="{{ session()->get('usuario')->oab; }}" disabled>
            </label>
            <br>
            <h1>Trocar senha</h1>
            <br>
